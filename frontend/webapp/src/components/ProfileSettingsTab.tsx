...
export const ProfileSettingsTab: React.FC<ProfileSettingsTabProps> = (
  ...
  const [showCurrentEmailOtpModal, setShowCurrentEmailOtpModal] = useState(false);
  const [newEmail, setNewEmail] = useState("");
  const [newEmailOtp, setNewEmailOtp] = useState("");
  const [showNewEmailOtpModal, setShowNewEmailOtpModal] = useState(false);
  const [showNewEmailInputModal, setShowNewEmailInputModal] = useState(false);
  const [currentEmailVerified, setCurrentEmailVerified] = useState(false);
  const [sendingEmailOtp, setSendingEmailOtp] = useState(false);
  const [verifyingEmailOtp, setVerifyingEmailOtp] = useState(false);
  const [resendingCurrentEmailOtp, setResendingCurrentEmailOtp] = useState(false);
  const [resendingNewEmailOtp, setResendingNewEmailOtp] = useState(false);
  ...
  const handleStartEmailChange = async () => {
    ...
    try {
      // Send OTP to current email address using the dedicated endpoint
      const response = await AuthService.sendCurrentEmailOtp();
      if (response) {
        setShowCurrentEmailOtpModal(true);
        toast({
          title: "OTP Sent",
          description: "Please enter the OTP sent to your current email address",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleResendCurrentEmailOtp = async () => {
    setResendingCurrentEmailOtp(true);
    try {
      const response = await AuthService.sendCurrentEmailOtp();
      if (response) {
        setCurrentEmailOtp(""); // Clear previous OTP
        toast({
          title: "OTP Resent",
          description: "A new OTP has been sent to your current email address",
        });
      }
    } catch (error: any) {
      toast({
      ...
      setResendingCurrentEmailOtp(false);
    }
  };
  ...
  const handleVerifyCurrentEmailOtp = async () => {
    ...
    setShowCurrentEmailOtpModal(false);
    setShowNewEmailInputModal(true); // Show modal to enter new email
    toast({
  ...
  const handleSendNewEmailOtp = async () => {
    if (!newEmail || newEmail.toLowerCase() === props.email?.toLowerCase()) {
      toast({
        title: "Invalid Email",
    ...
    setSendingEmailOtp(true);
    try {
      // Send OTP to new email address using the dedicated email change endpoint
      const response = await AuthService.sendNewEmailOtp(newEmail);
      if (response) {
        setShowNewEmailInputModal(false); // Close new email input modal
        setShowNewEmailOtpModal(true); // Show OTP verification modal
        toast({
          title: "OTP Sent",
          description: "Please enter the OTP sent to your new email address",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleResendNewEmailOtp = async () => {
    if (!newEmail) {
      toast({
        title: "Invalid Email",
        description: "Please enter a new email address first",
        variant: "destructive",
      });
      return;
    }

    setResendingNewEmailOtp(true);
    try {
      const response = await AuthService.sendNewEmailOtp(newEmail);
      if (response) {
        setNewEmailOtp(""); // Clear previous OTP
        toast({
          title: "OTP Resent",
          description: "A new OTP has been sent to your new email address",
        });
      }
    } catch (error: any) {
      toast({
  ...
  const handleVerifyNewEmailOtp = async () => {
    ...
    if (currentEmailOtp.length !== 6) {
      toast({
        title: "Invalid OTP",
        description: "Please verify your current email address first",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      // Verify both OTPs and change email address using the dedicated endpoint
      const response = await AuthService.verifyAndChangeEmail(
        currentEmailOtp,
        newEmail,
        newEmailOtp
      );
  ...
  const handleCancelEmailChange = () => {
    ...
    setCurrentEmailOtp("");
    setShowCurrentEmailOtpModal(false);
    setShowNewEmailInputModal(false);
    setShowNewEmailOtpModal(false);
  };
  ...
  return (
    <TabsContent value="settings" className="space-y-6">
                ...
                {!isChangingEmail && (
                  ...
                  >
                    {sendingEmailOtp ? (
                      ...
                      t("profilepage.settingsTab.changeEmail", "Change Email")
                    )}
                  </Button>
                )}
                {isChangingEmail && !currentEmailVerified && (
                  <Button
                    size="sm"
                    variant="outline"
      ...
      >
        <DialogContent>
          <DialogHeader>
            {t("profilepage.settingsTab.emailOtpTitle", "Enter OTP")}
          </DialogHeader>
          <p className="text-sm text-muted-foreground mb-4">
            Enter the OTP sent to your current email address: {props.email || ""}
          </p>
          <OtpInput
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Button
              ...
              disabled={currentEmailOtp.length !== 6 || verifyingEmailOtp}
            >
              {verifyingEmailOtp ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
          ...
          <div className="mt-3 text-center">
            <Button
              ...
              disabled={resendingCurrentEmailOtp}
              className="text-sm text-primary hover:text-primary/80"
            >
              {resendingCurrentEmailOtp ? (
                <>
                  <Loader2 className="h-3 w-3 animate-spin mr-1" />
                  Resending...
                </>
      ...
      {/* New Email Address Input Modal */}
      <Dialog
        open={showNewEmailInputModal}
        onOpenChange={(open) => {
          if (!open) {
            handleCancelEmailChange();
          }
      ...
      >
        <DialogContent>
          <DialogHeader>
            {t("profilepage.settingsTab.enterNewEmail", "Enter New Email Address")}
          </DialogHeader>
          <p className="text-sm text-muted-foreground mb-4">
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="newEmail">
                {t("profilepage.settingsTab.newEmailAddress", "New Email Address")}
              </Label>
              <Input
                id="newEmail"
                type="email"
            ...
            <div className="flex gap-2 mt-4">
              ...
              <Button
                ...
                disabled={sendingEmailOtp || !newEmail || newEmail.toLowerCase() === props.email?.toLowerCase()}
              >
                {sendingEmailOtp ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin mr-2" />
      ...
      >
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Enter the OTP sent to your new email address: {newEmail || ""}
          </p>
          <OtpInput
            value={newEmailOtp}
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Button
              ...
              disabled={newEmailOtp.length !== 6 || verifyingEmailOtp}
            >
              {verifyingEmailOtp ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin mr-2" />
          ...
          <div className="mt-3 text-center">
            <Button
              ...
              disabled={resendingNewEmailOtp}
              className="text-sm text-primary hover:text-primary/80"
            >
              {resendingNewEmailOtp ? (
                <>
                  <Loader2 className="h-3 w-3 animate-spin mr-1" />
                  Resending...
                </>
