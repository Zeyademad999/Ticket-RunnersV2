...
const Booking = () => {
  ...
  const [paymentConfig, setPaymentConfig] =
    useState<KashierPaymentConfig | null>(null);
  ...
  const eventData = apiEvent
    ? (() => {
        ...
        return {
          ...
          minimumAge:
            apiEvent.minimumAge ?? apiEventRaw.minimum_age ?? undefined,
  ...
  const ticketTiers = (eventData.ticketCategories || []).map((cat, index) => {
    const normalizedKey =
      cat.name?.toLowerCase().replace(/\s+/g, "_") || `tier_${index}`;
    ...
    return {
      ...
      price:
        typeof cat.price === "number"
          ? cat.price
          : parseFloat(String(cat.price || 0)) || 0,
  ...
  const proceedWithPayment = async () => {
    try {
      ...
      const selectedTier = ticketTiers.find((t) => {
        const qty =
          typeof quantities[t.key] === "number" && !isNaN(quantities[t.key])
            ? quantities[t.key]
            : 0;
        return qty > 0;
      });
      ...
      let ownerTicketsCount = currentTickets.filter(
        (t) => t.isOwnerTicket
      ).length;
      ...
      // Final validation before payment: check all tickets for phone conflicts
      // This is a critical check - must prevent self-assignment
      if (user?.mobile_number) {
        const userPhoneNormalized = normalizePhoneNumber(user.mobile_number);

        // Check all non-owner tickets
        for (const ticket of currentTickets) {
          if (!ticket.isOwnerTicket && ticket.mobile && ticket.mobile.trim()) {
            const ticketPhoneNormalized = normalizePhoneNumber(
              ticket.mobile.trim()
            );
            ...
            if (
              userPhoneNormalized &&
              ticketPhoneNormalized &&
              userPhoneNormalized === ticketPhoneNormalized
            ) {
              toast({
                title: t("booking.validationError", "Validation Error"),
                description:
                  t("booking.ownerPhoneConflict") ||
                  "You cannot assign a ticket to your own phone number. This ticket is already yours. Please use the 'This is my ticket' toggle for your own ticket.",
                variant: "destructive",
              });
              return;
            }
      ...
      const userPhoneNormalized = user?.mobile_number
        ? normalizePhoneNumber(user.mobile_number)
        : "";
      if (userPhoneNormalized) {
        for (let i = 0; i < currentTickets.length; i++) {
          const ticket = currentTickets[i];
          if (!ticket.isOwnerTicket && ticket.mobile) {
            const ticketMobileTrimmed = ticket.mobile.trim();
            if (ticketMobileTrimmed) {
              const ticketPhoneNormalized =
                normalizePhoneNumber(ticketMobileTrimmed);
              ...
              console.log("Phone validation check:", {
                ...
                userPhoneNormalized,
                ticketPhone: ticketMobileTrimmed,
                ticketPhoneNormalized,
                match: ticketPhoneNormalized === userPhoneNormalized,
              });

              if (
                ticketPhoneNormalized &&
                ticketPhoneNormalized === userPhoneNormalized
              ) {
                console.error("BLOCKED: Self-assignment detected!", {
                  ticketIndex: i,
                  userPhone: user.mobile_number,
                  ticketPhone: ticketMobileTrimmed,
                });
      ...
      const ticketDetails = currentTickets.map((t, index) => {
        ...
        const ticketTier =
          ticketTiers.find((tier) => tier.key === ticketTypeKey) ||
          selectedTier;
        ...
        // Don't auto-convert - validation should have caught this already
        // If somehow we get here with a matching phone, it's a bug, so we'll still block it
        const ticketMobile = (
          t.isOwnerTicket ? ownerMobile : t.mobile || ""
        ).trim();

        // Safety check: if mobile matches user's phone and it's not marked as owner, this is an error
        if (!t.isOwnerTicket && userPhoneNormalized && ticketMobile) {
          const ticketPhoneNormalized = normalizePhoneNumber(ticketMobile);
          if (ticketPhoneNormalized === userPhoneNormalized) {
            // This should never happen if validation worked, but log it and treat as owner to prevent issues
            console.error(
              "Validation bypass detected: ticket mobile matches user phone but not marked as owner",
              {
                ticketIndex: index,
                ticketMobile,
  ...
  // Track phone validation errors for each ticket
  ...
  const baseTicketPrice = ticketTiers.reduce((sum, t) => {
  ...
  if (eventData.isUnseated && isVip) {
    ...
    for (const tier of ticketTiers) {
      const tierQuantity =
        typeof quantities[tier.key] === "number" && !isNaN(quantities[tier.key])
          ? quantities[tier.key]
          : 0;
      if (tierQuantity > 0) {
  ...
  // Enhanced phone number normalization that handles all formats
  // Handles: +2001104484492, +201104484492, 01104484492, 201104484492, 1104484492
  const normalizePhoneNumber = (phone?: string): string => {
    ...
    // Handle Egyptian phone numbers - normalize all formats to 20XXXXXXXXXX (12 digits)
    // Case 1: +2001104484492 or 2001104484492 (with extra 0 after country code)
    if (normalized.startsWith("2001") && normalized.length === 13) {
      // Remove the extra 0: 2001104484492 -> 201104484492
      return "20" + normalized.substring(3);
    }

    // Case 2: +201104484492 or 201104484492 (standard format with country code)
    if (normalized.startsWith("201") && normalized.length === 12) {
      return normalized;
    }

    // Case 3: 01104484492 (Egyptian local format with leading 0)
    if (normalized.startsWith("0") && normalized.length === 11) {
      // Convert: 01104484492 -> 201104484492
      return "20" + normalized.substring(1);
    }

    // Case 4: 1104484492 (10 digits starting with 1, without leading 0)
    if (normalized.startsWith("1") && normalized.length === 10) {
      // Convert: 1104484492 -> 201104484492
      return "20" + normalized;
    }

    // Case 5: Already in international format (12+ digits starting with 20)
    if (normalized.startsWith("20") && normalized.length >= 12) {
      // If it's longer than 12, might have extra digits, take first 12
  ...
  const ensureOwnerTicket = (updatedTickets: typeof tickets, total: number) => {
    ...
    const unassignedIndexes = slice
      ...
      .filter(({ ticket }) => !ticket.isOwnerTicket && !isGuestAssigned(ticket))
      .map(({ idx }) => idx);
  ...
  const assignOwnerTicket = (targetIndex: number) => {
    ...
    // Clear phone errors for both the old and new owner tickets
    setPhoneErrors((prev) => {
      const updated = { ...prev };
      delete updated[currentOwnerIndex];
      delete updated[targetIndex];
      return updated;
    });
  ...
  // Enhanced validation that checks phone numbers in multiple formats
  // This prevents users from bypassing validation by changing country codes
  // The inputPhone comes from PhoneNumberInput as: countryCode + number (e.g., "+21301104484492")
  const checkPhoneNumberMatch = (
    inputPhone: string,
    userPhone: string,
    inputCountryCode?: string | null
  ): boolean => {
    if (!inputPhone || !userPhone) return false;

    // Normalize user's phone number (should be Egyptian, e.g., "201104484492")
    const normalizedUser = normalizePhoneNumber(userPhone);
    if (!normalizedUser) return false;

    // Remove all non-digits from input to get pure digits
    const inputDigits = inputPhone.replace(/\D+/g, "");
    if (!inputDigits) return false;

    // Extract the actual number part by removing the country code
    // PhoneNumberInput sends value as: dialCode + number (e.g., "+213" + "01104484492" = "+21301104484492")
    let numberPart = inputDigits;

    // If we have a country code, try to extract just the number part
    if (inputCountryCode) {
      const countryInfo = COUNTRY_DIAL_CODES.find(
        (c) => c.code === inputCountryCode
      );
      if (countryInfo) {
        // Get country dial code as digits only (e.g., "213" for Algeria)
        const countryDialCodeDigits = countryInfo.dial_code.replace(/\D+/g, "");

        // If input starts with this country code, remove it to get the number part
        if (inputDigits.startsWith(countryDialCodeDigits)) {
          numberPart = inputDigits.substring(countryDialCodeDigits.length);
        }
      }
    } else {
      // If no country code provided, try to detect and remove any country code
      ...
      for (const country of COUNTRY_DIAL_CODES) {
        ...
        if (
          inputDigits.startsWith(dialCodeDigits) &&
          inputDigits.length > dialCodeDigits.length
        ) {
          numberPart = inputDigits.substring(dialCodeDigits.length);
          break; // Use first match
        }
      }
    }

    // Now check if the number part (when normalized as Egyptian) matches user's number
    // The number part could be in formats like: "01104484492", "1104484492", etc.

    // Strategy 1: Normalize the number part as if it's an Egyptian number
    // This handles cases where user enters their Egyptian number with wrong country code
    const normalizedNumberPart = normalizePhoneNumber(numberPart);
    if (normalizedNumberPart && normalizedNumberPart === normalizedUser) {
      return true;
    }

    // Strategy 2: Also check the full input (in case it's already in correct format)
    const normalizedFullInput = normalizePhoneNumber(inputPhone);
    if (normalizedFullInput && normalizedFullInput === normalizedUser) {
      return true;
    }

    // Strategy 3: Try adding Egyptian country code to number part and check
    // This handles: numberPart = "01104484492", should become "201104484492"
    const withEgyptianCode = normalizePhoneNumber("20" + numberPart);
    if (withEgyptianCode && withEgyptianCode === normalizedUser) {
      return true;
    }

    // Strategy 4: Try with +20 prefix
    const withPlusEgyptianCode = normalizePhoneNumber("+20" + numberPart);
    if (withPlusEgyptianCode && withPlusEgyptianCode === normalizedUser) {
      return true;
    }
  ...
  // Validation function that checks if a phone number matches user's phone
  // This function always validates the actual number value, regardless of when it was set
  // countryCodeOverride: optional country code to use instead of reading from tickets state
  const validatePhoneNumberForTicket = (
    ticketIndex: number,
    ...
    // Skip validation if it's the owner ticket
    if (tickets[ticketIndex]?.isOwnerTicket) {
      return true;
    }
    ...
    // Skip validation if user doesn't have a mobile number
    if (!user?.mobile_number) {
      return true;
    }

    const trimmedValue = phoneNumber.trim();
    // Use override if provided, otherwise get from tickets state
    const ticketCountryCode =
      countryCodeOverride !== undefined
        ? countryCodeOverride
        : tickets[ticketIndex]?.mobileCountryCode;

    // Debug logging for validation
    console.log("Validating phone number:", {
      ticketIndex,
      inputPhone: trimmedValue,
      countryCode: ticketCountryCode,
      userPhone: user.mobile_number,
      usingOverride: countryCodeOverride !== undefined,
    });

    // Use enhanced matching that considers country codes
    const isMatch = checkPhoneNumberMatch(
      trimmedValue,
      user.mobile_number,
      ticketCountryCode
    );
    ...
    if (isMatch) {
      ...
      // Set error state for this ticket
      setPhoneErrors((prev) => ({
        ...prev,
        [ticketIndex]: errorMessage,
      }));
  ...
  const updateTicket = (
    ...
    // Real-time validation for phone number assignment
    if (
      field === "mobile" &&
      typeof value === "string" &&
      user?.mobile_number
    ) {
      // Get current country code for this ticket to pass to validation
      const currentTicket = tickets[index];
      const countryCode = currentTicket?.mobileCountryCode;

      // Validate the phone number - this will set errors if needed
      // Pass the country code so validation can properly extract the number part
      const isValid = validatePhoneNumberForTicket(index, value, true);
      ...
      // If validation fails, don't update the ticket with this value
      if (!isValid) {
        return;
      }
    }
  ...
  // Re-validate all phone numbers when country codes change
  // This ensures validation always runs, even when country code changes without number change
  // This prevents loopholes where users can bypass validation by changing country code
  // Use a ref to track previous country codes to avoid unnecessary re-validations
  const prevCountryCodesRef = useRef<string>("");

  useEffect(() => {
    if (!user?.mobile_number) return;

    // Create a string representation of all country codes
    const currentCountryCodes = tickets
      .map((t, i) => `${i}-${t.mobileCountryCode || "EG"}`)
      .join(",");

    // Only validate if country codes actually changed
    if (currentCountryCodes !== prevCountryCodesRef.current) {
      prevCountryCodesRef.current = currentCountryCodes;

      tickets.forEach((ticket, index) => {
        // Only validate non-owner tickets with phone numbers
        if (!ticket.isOwnerTicket && ticket.mobile && ticket.mobile.trim()) {
          validatePhoneNumberForTicket(index, ticket.mobile, false);
        }
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    // Track country codes for all tickets - if any changes, re-validate
    tickets.map((t, i) => `${i}-${t.mobileCountryCode || "EG"}`).join(","),
    user?.mobile_number,
  ]);
  useEffect(() => {
    ...
    <div className="min-h-screen bg-gradient-dark">
      <Dialog open={showOwnerLockDialog} onOpenChange={setShowOwnerLockDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {t("booking.ownerTicketLockedTitle", "Ticket Locked to You")}
            </DialogTitle>
          </DialogHeader>
          <p>
            {t(
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {eventData.minimumAge && (
              <div className="mb-4 text-sm text-destructive">
                {t("booking.ageRestrictionNotice", {
                  age: eventData.minimumAge,
                })}
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {eventData.artist_name && eventData.artist_name.trim() && (
                        ...
                        return (
                          <div
                            ...
                            className={`p-4 rounded-lg transition-all duration-200 hover:shadow-md ${
                              tier && (tier as any).color
                                ? ""
                                : colorClass || ""
                            }`}
                            style={
                              Object.keys(colorStyle).length > 0
                                ? colorStyle
                                : undefined
                            }
                          >
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      {addOrder.map((ticketType, index) => {
                        ...
                        const requiresInternationalEmail =
                          requiresEmailForTicket(ticket);
                              ...
                              <div className="space-y-2">
                                ...
                                {!ticket.isOwnerTicket ? (
                                    ...
                                    <div className="space-y-2">
                                      <PhoneNumberInput
                                        ...
                                        onDialCodeChange={(_, country) => {
                                          ...
                                          // CRITICAL: Re-validate phone number when country code changes
                                          // This prevents loopholes where changing country code bypasses validation
                                          // Pass the new country code directly to avoid state timing issues
                                          if (
                                            ticket.mobile &&
                                            ticket.mobile.trim()
                                          ) {
                                            console.log(
                                              "Re-validating after country code change:",
                                              {
                                                index,
                                                phone: ticket.mobile,
                                            ...
                                            validatePhoneNumberForTicket(
                                              index,
                                              ticket.mobile,
                                              true,
                                              newCountryCode // Pass new country code directly
                                            );
                                          }
                                        }}
                                        required
                                  ...
                                  <div className="text-sm text-muted-foreground bg-muted/50 p-3 rounded-lg">
                                    ...
                                    <p>
                                      {t("booking.ownersTicketDescription")}
                                    </p>
                                  </div>
                                )}
                              </div>
