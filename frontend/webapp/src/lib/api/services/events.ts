...
export class EventsService {
  ...
  private static transformEventDetails(apiData: any): EventData {
    return {
      ...
      title: apiData.title || "",
      artist_name: (apiData.artist_name && apiData.artist_name.trim()) || (apiData.artistName && apiData.artistName.trim()) || undefined,
      ...
      layoutImageUrl: (() => {
        if (apiData.venue_layout_image) {
          const url = this.getFullImageUrl(apiData.venue_layout_image);
          console.log("Venue layout image from venue_layout_image:", apiData.venue_layout_image, "->", url);
          return url;
        }
        if (apiData.venue_layout_image_url) {
          const url = this.getFullImageUrl(apiData.venue_layout_image_url);
          console.log("Venue layout image from venue_layout_image_url:", apiData.venue_layout_image_url, "->", url);
          return url;
        }
        if (apiData.venue?.layout_image?.path) {
          const url = this.getFullImageUrl(apiData.venue.layout_image.path);
          console.log("Venue layout image from venue.layout_image.path:", apiData.venue.layout_image.path, "->", url);
          return url;
        }
        console.log("No venue layout image found in API data");
        return undefined;
      })(),
