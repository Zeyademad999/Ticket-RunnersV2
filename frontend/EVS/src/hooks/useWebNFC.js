...
const NFC_BRIDGE_URL = (process.env.REACT_APP_NFC_BRIDGE_URL || 'http://localhost:8765').replace(/\/$/, '');
const NFC_WS_URL = (process.env.REACT_APP_NFC_WS_URL || 'ws://localhost:9090').replace(/\/$/, '');
...
export const useWebNFC = () => {
  ...
  const autoScanCallbacksRef = useRef(new Set());

  // Check if NFC Bridge Server is running
  const checkBridgeServer = useCallback(async () => {
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          if (data.type === 'SCAN' && data.uid) {
            ...
            let callbackIndex = 0;
            autoScanCallbacksRef.current.forEach((callback) => {
              callbackIndex++;
              try {
                console.log(`   → Calling auto-scan callback ${callbackIndex}`);
                callback(data.uid);
              } catch (err) {
                console.error(`   ❌ Error in auto-scan callback ${callbackIndex}:`, err);
              }
            });
  ...
  }, [checkSupport, checkBridgeServer]);
  ...
  const onCardScanned = useCallback((callback) => {
